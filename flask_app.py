...
#------------------------------------------------------------------#
#==========================================================================================#
#saving filename of UPLOADED FILE 
def sav_name(n):
    with open("ext/filename.txt", "w") as ff:
        ff.write(n)
#------------------------------------------------------------------#
#=============================================================================================#
#preprocessing data of UPLOADED FILE        
def preprocess_data():
    dataset = pd.read_csv('ext/Churn_Modelling.csv')
    ffr = open("ext/filename.txt", "r")
    upl_file = ffr.read()
...
def model_2(cid1):
    ...
    data_re.to_csv('ext/data_re.csv')
    X = dataset.iloc[:, 3:14].values
    test=pd.read_csv('ext/testtest1.csv')
    cid1 = int(cid1)
...
def search(cid):
    with open('ext/testtest1.csv') as file:
...
def uploader_file():
   if request.method == 'POST':
      ...
      model = load_model('ext/my_model.h5')
      ...
      dff.to_csv('ext/testtest1.csv')  # output file
      ...
      return render_template('template_up.html',  dropdown_list=dropdown_list)
...
def input_num():
    x = request.form["in"]
    line = pd.read_csv("ext/testtest1.csv").shape[0]
    ...
    with open("ext/testtest1.csv") as file:
        ...
        for row in allRead:
            if lineCount == 0:
                ...
                lineCount += 1
    lss=list(map(lambda x: float(x*100),list(pd.read_csv("ext/testtest1.csv")['Exited'][:y].copy())))  
    return render_template('template_up_percent.html', outList = ls, value_list=lschurn,values_res=lss )
...
def input_num_default():
    ...
    return render_template('template_def_percent.html', outList = ls , value_list=lschurn , values_res=lss)
...
def uploader_default_file():
      ...
      return render_template('template_def.html',  dropdown_list_2=dropdown_list_2)
...
def specific_default(dropdown_2):
    ...
    values_res[2] = round(values_res[2],2)
    values_res[3] = round(values_res[3],2)
    colors = [ "#d9c3ff", "#46BFBD", "#FDB45C" , "#ABCDEF"]
    return render_template('chart_meter.html', set=zip(values_res, labels_res, colors),firstname=x, rownum=rownum, ccid=ccid, surname=surname, creditscore=creditscore, geo=geo, gender=gender, age=age, tenure=tenure, balance=balance, numpro=numpro, hascard = hascard, activemem = activemem, salary = salary, secondname = values_res[0] , secondname1 = values_res[1] , secondname2 = values_res[2] , secondname3 = values_res[3] ,labels_res=labels_res,values_res=values_res, values=values, labels=labels)
...
def do_admin_login():
    error = None
    if request.form['username'] != 'elyt-ai@demo' or request.form['password'] != 'capslock':
...
def predict(z):
     ...
     cid1 = z 
     test3 = model_2(cid1)
     model2 = load_model('ext/my_model2.h5')
